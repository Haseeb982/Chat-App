...
const NewDm = () => {
    ...
    const searchContact = async (searchTerm) => {
        try {
            if (searchTerm.length > 0) {
                console.log("frontend before post");
                ...
                if (response.status === 200 && response.data.contacts) {                    
                    console.log("data is here", response.data.contacts)
    ...
    const selectNewContact = async (contact) => {
        try {
            ...
            console.log("firstname is here", contact.firstName)
            setopenNewModelContact(false)
            SetSelectedChatType("contact")
            SetSelectedChatData(contact)
            console.log("frontend contact data:", contact)
            setsearchedContacts([])
            console.log("data is here", useAuthStore.getState().SelectedChatData)
        } else {
            console.log("error in serhitems")
        }          
        } catch (error) {
